...
import { createElement, useEffect as useEffect4, useRef as useRef4, useState as useState2 } from "react";

// src/context/orb-scene-context.ts
import { createContext, useCallback, useContext, useMemo, useRef } from "react";
var OrbSceneContext = createContext(null);
...
function useOrbSceneProvider({
  ...
  const orbConfigsRef = useRef(new Map);
  const imperativeRendererRef = useRef(null);
  ...
  const registerOrbConfig = useCallback((id, config) => {
    orbConfigsRef.current.set(id, config);
    syncToRenderer();
  }, [syncToRenderer]);
  const unregisterOrbConfig = useCallback((id) => {
...
function Grain({ intensity = 0.35, className, style }) {
  ...
  const renderNoise = useCallback2(() => {
...
import { useEffect as useEffect3, useId, useMemo as useMemo2, useState } from "react";
...
function Orb({
  ...
  const fallbackOrbIndex = useMemo2(() => {
    let hash = 0;
  ...
  const [blobAnimProps, setBlobAnimProps] = useState(null);
  useEffect3(() => {
...
function OrbScene({
  ...
  const [resolvedRenderer, setResolvedRenderer] = useState2(renderer === "auto" ? "css" : renderer);
  useEffect4(() => {
    if (renderer === "auto") {
  ...
  const presetOrbs = presetData?.points.map((point, index) => /* @__PURE__ */ jsxDEV4(Orb, {
    color: point.color,
  ...
  const isImperative = resolvedRenderer !== "css";
  return /* @__PURE__ */ jsxDEV4(OrbSceneContext.Provider, {
    value: contextValue,
    ...
    }, isImperative ? /* @__PURE__ */ jsxDEV4(ImperativeScene, {
      rendererType: resolvedRenderer
    }, undefined, false, undefined, this) : null, presetOrbs, children, !isImperative && resolvedGrain > 0 ? /* @__PURE__ */ jsxDEV4(Grain, {
